...
def rewrite_imports(src: str, allowed_roots: Set[str], pkg_name: str) -> Tuple[str, bool]:
    """
    ...
    for node in tree.body:
        ...
        if isinstance(node, ast.ImportFrom) and node.level == 0 and node.module:
            ...
            if first_seg in allowed_roots:
                ...
                new_line = original.replace(f"from {node.module} import", f"from {new_module} import", 1)
        ...
        # import x, y as z
            ...
            for alias in node.names:
                ...
                if first_seg == pkg_name:
                    continue
                if first_seg in allowed_roots:
                    new_line = new_line.replace(f"import {name}", f"import {pkg_name}.{name}")
            patch_line(line_idx, original, new_line)
