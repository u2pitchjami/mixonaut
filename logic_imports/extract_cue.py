...
@with_child_logger
def get_audio_base_name(cue_path, logger=None):
...
@with_child_logger
def split_cue_and_convert_ffmpeg(cue_path, logger=None):
    """
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="cue_split_ffmpeg_") as temp_out:
            ...
            for idx, (start, end) in enumerate(zip(points[:-1], points[1:]), 1):
                ...
                if logger:
                    logger.debug(f"[FFMPEG] Découpe {track_num}: {start} → {end} en .{ext}")
                subprocess.run(cmd, check=True)
