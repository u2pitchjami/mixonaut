...
def fetch_tracks(
    ...
    if missing_field:
        allowed_fields = {
            "bpm", "energy_level", "mood", "beat_intensity",
            "initial_key", "rg_track_gain", "genre"
        }
...
@with_child_logger
def get_file_sha1_by_track(track_id: int, logger=None) -> Optional[str]:
    row = select_one(SQL_GET_SHA1_BY_TRACK, (track_id,), logger=logger)
    return row[0] if row else None

@with_child_logger
def list_candidate_tracks_same_sha1(file_sha1: str, exclude_track_id: int, logger=None) -> List[int]:
    rows = select_all(SQL_FIND_TRACKS_WITH_SAME_SHA1_EXCEPT, (file_sha1, exclude_track_id), logger=logger)
...
@with_child_logger
def get_audio_hash_sha256_by_track(track_id: int, logger=None) -> Optional[str]:
    row = select_one(SQL_GET_SHA256_BY_TRACK, (track_id,), logger=logger)
    return row[0] if row else None

@with_child_logger
def list_candidate_tracks_same_sha256(audio_hash_sha256: str, exclude_track_id: int, logger=None) -> List[int]:
    rows = select_all(SQL_FIND_TRACKS_WITH_SAME_SHA256_EXCEPT, (audio_hash_sha256,), logger=logger)
    return [r[0] for r in rows] if rows else []