...
from utils.config import ESSENTIA_SAV_JSON  # dossier destination des JSON archivés
...
def find_latest_sav_json(track_id: int, logger: logging.Logger) -> Optional[Path]:
    """
    ...
    if not shard.exists():
        ...
        return None
    candidates: List[Path] = sorted(shard.glob(f"{track_id}_*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
    if not candidates:
...
def duplicate_essentia_json(
    ...
    *,
    ...
    if donor_json is None:
        logger.warning("♻️ Reuse annulé: JSON donneur introuvable (track_id=%s)", donor_track_id)
        return None
    ...
    if patch_track_id:
        try:
            with open(dest_json, "r", encoding="utf-8") as f_in:
                data = json.load(f_in)
            ...
            logger.exception("Patch du track_id dans %s a échoué (on garde la copie telle quelle)", dest_json)

    return dest_json


def load_essentia_json(json_path: Path, logger: None) -> Optional[dict]:
    """Charge un JSON Essentia en dict. Retourne None en cas d'erreur."""
    try:
