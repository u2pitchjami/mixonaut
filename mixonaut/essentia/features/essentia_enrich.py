...
def enrich_features(track_features, logger=None):
    try:
        track_features["bpm"] = sanitize_value(track_features.get("bpm"), "bpm", logger=logger)
        track_features["rg_track_gain"] = sanitize_value(track_features.get("rg_track_gain"), "rg_gain", logger=logger)

        track_features["beat_intensity"] = calculate_beat_intensity(track_features, logger=logger)
        ...
        if best_key_data:
            ...
            track_features["initial_key"] = sanitize_value(camelot, "key", logger=logger)
