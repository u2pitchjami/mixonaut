"""2025-08-20 - script de récap des modifs beets sur une période donnée."""
...
from mixonaut.utils.config import BEETS_RECAP_DIR
...
def load_snapshot(file_path: str) -> dict[str, tuple[str, str]]:
    """
    Charge un snapshot provenant d'un fichier de format texte spécifique.

    Args:
        file_path (str): Chemin vers le fichier de snapshot à charger.

    Returns:
        dict[str, tuple[str, str]]: Dictionnaire contenant les chemins des fichiers sous clé
            et leurs données respectives. Les données sont au format (mtime, added).
    """
    data = {}
...
def generate_beet_change_log(
    old_file: str,
    new_file: str,
) -> dict[str, list[str]]:
    """
    Génère un rapport de changement des fichiers Beets entre deux snapshots.

    Args:
        old_file (str): Chemin vers le fichier de snapshot ancien.
        new_file (str): Chemin vers le fichier de snapshot nouveau.

    Returns:
        dict[str, list[str]]: Dictionnaire contenant les informations sur les modifications apportées aux fichiers
            Beets, sous forme de liste de chemins de fichiers modifiés ou supprimés.
    """
    ...
    if not added and not modified and not removed:
        logger.info("🎹 Aucun mouvement sur la période")
    else:
...
def auto_generate_beet_change_log(period: str = "all"):
    ...
    Args:
        period (str): La période à considérer, ou 'all' pour toutes les périodes. Par défaut, 'all'.

    Returns:
        dict[str, list[str]]: Le rapport de changement des fichiers Beets.
    """
...
def get_last_two_snapshots(period: str = "all") -> tuple[str, str] | None:
    """
    Récupère les deux derniers fichiers snapshot dans le dossier BEETS_RECAP_DIR, filtrés selon la période : "all",
    "week", "month".
...
def export_change_log_markdown(
    album_changes: dict[str, set[str]], output_dir: str
) -> str:
    """
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        ...
        for t in sorted(grouped):
            emoji = {"added": "🟢", "modified": "🔄", "removed": "🔴"}.get(t, "📁")
...
def main(period=all, nosnapshot=False, markdown=False):
    ...
    logger.info("🏁 BEETS RECAP : TERMINE !! \n\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="all",
        help="Période à analyser",
    )
    parser.add_argument(
        "--nosnapshot", action="store_true", help="Pas de Snapshot, juste le comparatif"
    )
    parser.add_argument(
        "--markdown", action="store_true", help="Génère un fichier markdown"
    )
