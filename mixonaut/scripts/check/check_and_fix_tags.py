...
from db.analyse.essentia_queries import get_all_track_ids, get_audio_features_by_id
from db.beets.db_beets_queries import get_item_field_value
from process_analyse.retro_beets.write_tags import write_tags_docker, docker_metaflac_cmd
from process_analyse.retro_beets.sync_beets_from_essentia import build_sync_fields
...
def process_all_tracks(dry_run: bool = False, track_id: int = None, nb_limit: int = 0):
    ...
    for i, track_id in enumerate(track_ids, 1):
        count += 1
        if updated >= 1:
            logger.info(f"▶️  Analyse : {track_id} - [{format_nb(count, logger=logger)}/{format_nb(total, logger=logger)}] ({format_percent(count, total, logger=logger)}) - {format_nb(updated, logger=logger)} updated ({format_percent(updated, total, logger=logger)})")
        else:
            logger.info(f"▶️  Analyse : {track_id} - [{format_nb(count, logger=logger)}/{format_nb(total, logger=logger)}] ({format_percent(count, total, logger=logger)})")
